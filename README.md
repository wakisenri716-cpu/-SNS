...
- **MaaS連携**（`backend/src/services/maasProvider.ts`）：リールに紐づく場所情報までの、徒歩＋電車の所要時間を表示します。`GOOGLE_MAPS_API_KEY` が設定されていれば **Google Maps Platform の実API**（Geocoding + Directions）で実際の経路データを計算し、未設定の場合は自動的に「モックデータ」（決定的な仮の所要時間）にフォールバックします。モック使用時は画面上に必ず「交通情報は仮データ」と表示されるので、ユーザーはどちらのデータか判別できます。設定方法は次の「Google Maps連携のセットアップ」を参照してください。
...
3. 左メニュー「APIとサービス → ライブラリ」から以下の2つを検索して有効化する
   ...
   - **Directions API**（2地点間の経路・所要時間の取得。`mode=transit` で電車・バス経路も取得）
...
設定後、自治体アカウントの管理画面（`/dashboard`）で「MaaS（交通案内）起点駅」欄に最寄り駅・バス停名を入力すると、その地点を起点としてリールの位置情報までの実際の経路（徒歩＋電車）が計算されるようになります。未入力の場合や、キー未設定・API呼び出し失敗時は自動的にモックデータが使われます。
