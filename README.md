...
MaaS機能を実際の地図・経路データで動かすには、Google Maps Platform の **Geocoding API** と **Directions API** を有効化したAPIキーが必要です。キーがない状態でもアプリは正常に動作します（自動的にモックデータにフォールバックします）。
...
3. 左メニュー「APIとサービス → ライブラリ」から以下の2つを検索して有効化する
   - **Geocoding API**（地名・住所 → 緯度経度への変換）
   - **Directions API**（2地点間の距離・所要時間の取得）
4. 「APIとサービス → 認証情報」→「認証情報を作成」→「APIキー」でキーを発行する
5. 発行したキーを選択し、「APIの制限」で上記2つのAPI（Geocoding API, Directions API）のみに制限する（不要な課金・悪用防止のため必須）。「アプリケーションの制限」は、サーバーサイドからのみ呼び出すキーなので「なし」のままで問題ありません
